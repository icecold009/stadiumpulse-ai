...
export default function OperatorContextBanner() {
    return (
        <section
            className="mb-5 overflow-hidden rounded-xl border border-border bg-surface px-4 py-3"
            aria-label="Demo context and decision loop"
        >
            <div className="flex flex-wrap items-center gap-x-3 gap-y-2">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-accent-strong/45 bg-accent-soft px-2.5 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-accent-strong">
                    <Activity aria-hidden="true" className="h-3 w-3" /> Simulated live
                </span>
                <p className="flex flex-wrap items-center gap-2 text-xs font-semibold text-foreground sm:text-sm">
            ...
            <p className="mt-2 flex items-start gap-1.5 text-xs leading-5 text-text-muted">
                <ShieldCheck aria-hidden="true" className="mt-0.5 h-3.5 w-3.5 shrink-0 text-accent-strong" />
