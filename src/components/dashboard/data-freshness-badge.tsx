...
export default function DataFreshnessBadge({ freshness }: { freshness: DataFreshness }) {
    ...
    return (
        <span role="status" aria-label={`${config.label}. ${age}`} className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${config.className}`}>
            <Icon aria-hidden="true" className="h-3 w-3" />
            <span>{config.label}</span>
            <span className="font-normal normal-case tracking-normal text-text-muted">{age}</span>
