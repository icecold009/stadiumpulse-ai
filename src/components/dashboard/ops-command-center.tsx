...
export default function OpsCommandCenter({ initialSnapshot }: Props) {
    ...
    return (
        <div className="space-y-7">
            <PageHeader
                eyebrow="Venue command center"
                title="Operations situation room"
                description="Identify the highest-risk conditions, understand the evidence, and keep the operational decision human-controlled."
                actions={<>
                    <DataFreshnessBadge freshness={snapshot.freshness} />
                    <button type="button" onClick={() => void refresh()} disabled={isRefreshing} aria-keyshortcuts="R" className="control button-secondary">
                        <RefreshCw aria-hidden="true" className={`h-3.5 w-3.5 ${isRefreshing ? "animate-spin" : ""}`} /> {isRefreshing ? "Refreshing…" : "Refresh"}
                    </button>
                </>}
            />

            <section className={`panel hidden p-5 sm:p-6 md:block ${priorityAlert ? "border-status-critical/55 bg-status-critical/8" : "border-status-ok/35 bg-status-ok/6"}`} aria-labelledby="priority-heading">
                <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                        <p className="eyebrow flex items-center gap-2 text-text-muted">
                    ...
                    <button type="button" onClick={() => setIsAlertFocusMode((current) => !current)} aria-pressed={isAlertFocusMode} className="control button-secondary">
                ...
                {priorityAlert ? (
                    <div className="mt-4 grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                        {snapshot.alerts.slice(0, 2).map((alert) => (
                            <article key={alert.id} className="rounded-xl border border-border bg-surface-muted/45 p-4">
            ...
            {priorityAlert ? <section data-alert-queue="true" className="panel border-status-critical/55 bg-status-critical/8 p-4 md:hidden" aria-labelledby="mobile-emergency-heading">
                <p className="eyebrow text-status-critical">Mobile emergency view</p>
                ...
                <div className="mt-4 grid grid-cols-1 gap-2">
                    <button type="button" onClick={() => void handleAlertAction(priorityAlert.id, "accept")} disabled={pendingActionId !== null || priorityAlert.operatorDecision === "accepted"} className="control button-secondary border-status-ok/45 text-status-ok">Accept recommendation</button>
                    <button type="button" onClick={() => void handleAlertAction(priorityAlert.id, "reject")} disabled={pendingActionId !== null || priorityAlert.operatorDecision === "rejected"} className="control button-secondary border-status-warn/45 text-status-warn">Reject recommendation</button>
                    <button type="button" onClick={() => void handleAlertAction(priorityAlert.id, "handled")} disabled={pendingActionId !== null} className="control button-secondary">Mark incident handled</button>
                </div>
            ...
            {error ? <div role="alert" className="rounded-[10px] border border-status-warn/45 bg-status-warn/10 px-4 py-3 text-sm text-status-warn">{error}</div> : null}

            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(360px,0.75fr)]">
                <Panel aria-labelledby="risk-heading" className="p-5">
                    <SectionHeader eyebrow="Risk map" title="Zones needing attention" action={<span className="text-xs text-text-muted">{snapshot.zones.length} monitored zones</span>} />
                    <div className="mt-5 grid gap-3 sm:grid-cols-2">
                        {topRisks.length === 0 ? <p className="rounded-xl border border-border bg-surface-muted/40 p-4 text-sm text-text-muted">No zone telemetry is available.</p> : topRisks.map((zone) => (
                            <article key={zone.id} className="rounded-xl border border-border bg-surface-muted/40 p-4">
                                ...
                                <div className="mt-4 flex items-center justify-between text-xs">
                                    ...
                                    <button type="button" onClick={() => askCopilot({ question: `What should the operator know about ${zone.label} right now?`, zoneId: zone.id, venueId: zone.venueId })} className="control button-ai min-h-8 rounded-lg px-2.5 py-1 text-[11px]">Ask Copilot</button>
                                </div>
                            </article>
                        ))}
                    </div>
                ...
                {!isAlertFocusMode ? <Panel aria-labelledby="gate-heading" className="p-5">
                    <SectionHeader eyebrow="Entry flow" title="Gate throughput" action={<span className="text-xs text-text-muted">Latest readings</span>} />
                    <div className="mt-5 space-y-3">
                        {snapshot.gates.length === 0 ? <p className="rounded-xl border border-border bg-surface-muted/40 p-4 text-sm text-text-muted">No gate readings are available.</p> : snapshot.gates.slice(0, 6).map((gate) => (
                            <div key={gate.id} className="flex items-center justify-between gap-3 rounded-xl border border-border bg-surface-muted/40 px-3 py-3">
                                <div>
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button type="button" onClick={() => askCopilot({ question: `What should the operator know about ${gate.label} throughput right now?`, gateId: gate.id, venueId: gate.venueId })} className="control button-ai min-h-8 rounded-lg px-2.5 py-1 text-[11px]">Ask</button>
                                </div>
                            </div>
                        ))}
                    </div>
                </Panel> : <aside className="panel border-status-critical/35 bg-status-critical/6 p-5" aria-label="Incident focus mode">
                    <p className="eyebrow text-status-critical">Incident focus active</p>
                    ...
                    <button type="button" onClick={() => setIsAlertFocusMode(false)} className="control button-secondary mt-4">Show secondary panels</button>
                </aside>}
            </div>

            <Panel id="alert-queue" data-alert-queue="true" tabIndex={-1} className="p-5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent" aria-labelledby="incident-heading">
                <SectionHeader eyebrow="Decision queue" title="Open incidents and recommendations" action={<a href={initialSnapshot.selectedVenueId ? `/ops/alerts?venueId=${initialSnapshot.selectedVenueId}` : "/ops/alerts"} className="control text-xs font-semibold text-accent-strong hover:text-foreground">Open full alert queue</a>} />
                <div className="mt-5 grid gap-4 lg:grid-cols-2">
                    {snapshot.alerts.length === 0 ? <p className="rounded-xl border border-border bg-surface-muted/40 p-4 text-sm text-text-muted">No open incidents in this venue scope.</p> : snapshot.alerts.slice(0, 4).map((alert) => (
            ...
            {!isAlertFocusMode ? <Panel aria-labelledby="decision-heading" className="p-5">
                <SectionHeader eyebrow="Human control" title="Decision timeline" action={<span className="text-xs text-text-muted">Recorded operator outcomes</span>} />
                {snapshot.decisionTimeline.length === 0 ? <p className="mt-4 rounded-xl border border-border bg-surface-muted/40 p-4 text-sm text-text-muted">No operator decisions recorded in the current open-alert snapshot.</p> : <ol className="mt-4 space-y-3">
                    {snapshot.decisionTimeline.map((event) => <li key={`${event.alertId}:${event.decidedAt}`} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface-muted/40 px-3 py-3 text-sm">
...
function MetricCard({ label, value, unit, icon, status, statusLabel, nextAction, recordedAt }: { label: string; value: string; unit: string; icon: React.ReactNode; status: "ok" | "warn" | "critical"; statusLabel: string; nextAction: string; recordedAt: string | null }) {
    return <article className="panel min-h-full bg-surface-muted/45 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.12em] text-text-muted">{icon}{label}</div><p className="mono-data mt-3 text-2xl font-bold tracking-tight text-foreground">{value}<span className="ml-2 font-sans text-xs font-medium tracking-normal text-text-muted">{unit}</span></p><div className="mt-3 flex flex-wrap items-center gap-2"><StatusBadge status={status} label={statusLabel} /><span className="text-[11px] text-text-muted">Next: {nextAction}</span></div><p className="mt-2 text-[11px] text-text-muted">{recordedAt ? `Observed ${new Date(recordedAt).toLocaleString()}` : "Observation unavailable"}</p></article>;
}
